/**
 * npm run templates — builds the content templates TAC Solutions fills in (T-006, FR-13).
 * Output: templates/risksense-content-template.xlsx (4 sheets) and templates/sample-content.json.
 * Column names here ARE the import contract: datasets.service (T-024) validates uploads against them.
 */
import ExcelJS from 'exceljs';
import { mkdirSync, writeFileSync } from 'node:fs';
import { join } from 'node:path';

const OUT_DIR = join(process.cwd(), 'templates');
...
async function build() {
  ...
  [
    'RiskSense AI — content template (v1, 2026-09-13)',
    '',
  ...
  for (const [sheetName, cols] of Object.entries(TEMPLATE_COLUMNS)) {
    ...
    for (const row of SAMPLE[sheetName as keyof typeof SAMPLE]) ws.addRow(cols.map(([key]) => (row as Record<string, unknown>)[key] ?? ''));
  }

  const xlsxPath = join(OUT_DIR, 'risksense-content-template.xlsx');
  await wb.xlsx.writeFile(xlsxPath);

  const jsonPath = join(OUT_DIR, 'sample-content.json');
  writeFileSync(jsonPath, JSON.stringify({ version: 1, ...SAMPLE }, null, 2));
