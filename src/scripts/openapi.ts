...
const AuthTenant = z
  ...
  .openapi('AuthTenant');
const AccessMode = z.enum(['standard', 'public_demo_sandbox']).openapi('AccessMode');
...
registry.registerPath({ method: 'post', path: '/audit-logs/archive', security: secured, request: { body: { content: { 'application/json': { schema: ArchiveAuditBody } } } }, responses: { 201: { description: 'Bounded clear-text audit-payload export plus immutable manifest/hash; session entity ids are always non-secret fingerprints (standard system_administrator with fullAudit)', content: { 'application/json': { schema: Envelope(z.object({ manifest: z.record(z.unknown()), records: z.array(z.record(z.unknown())) })) } } }, 403: { description: 'Forbidden for public_demo_sandbox because the export contains raw audit payloads', content: { 'application/json': { schema: ErrorEnvelope } } } } });
...
registry.registerPath({ method: 'post', path: '/assessments', security: secured, request: { body: { content: { 'application/json': { schema: StartBody } } } }, responses: { 201: { description: 'Started; public-demo visitor assessments are isolated to the creating session', content: { 'application/json': { schema: Envelope(Turn) } } }, 429: { description: 'Shared public-demo Requestor AI/start budget exhausted (240 operations per tenant/user per 10 minutes)', content: { 'application/json': { schema: ErrorEnvelope } } } } });
...
const AssessmentSummary = z.object({ averageConfidence: z.number().min(0).max(100).nullable() }).openapi('AssessmentSummary');
registry.registerPath({ method: 'get', path: '/assessments', security: secured, request: { query: AssessmentListQuery }, responses: { 200: { description: 'Review dashboard (DASH-01): scoped, filtered, paginated; public-demo Requestors see only the current session and sandbox operator roles see only untagged seeded history', content: { 'application/json': { schema: Envelope(z.object({ items: z.array(AssessmentListItem), total: z.number(), page: z.number(), limit: z.number(), pages: z.number(), counts: AssessmentCounts, summary: AssessmentSummary })) } } } } });
...
registry.registerPath({ method: 'get', path: '/assessments/{id}', security: secured, request: { params: idp, query: UnmaskQuery }, responses: { 200: { description: 'Assessment; unmask=true is forbidden for public_demo_sandbox', content: { 'application/json': { schema: Envelope(Assessment) } } } } });
registry.registerPath({ method: 'get', path: '/assessments/{id}/messages', security: secured, request: { params: idp, query: UnmaskQuery }, responses: { 200: { description: 'Transcript; unmask=true is forbidden for public_demo_sandbox', content: { 'application/json': { schema: Envelope(z.array(Any)) } } } } });
registry.registerPath({ method: 'post', path: '/assessments/{id}/persona', security: secured, request: { params: idp, body: { content: { 'application/json': { schema: AssessmentPersonaBody } } } }, responses: { 200: { description: 'Persona set/overridden', content: { 'application/json': { schema: Envelope(Turn) } } }, 429: { description: 'Shared public-demo Requestor AI/start budget exhausted', content: { 'application/json': { schema: ErrorEnvelope } } } } });
registry.registerPath({ method: 'post', path: '/assessments/{id}/messages', security: secured, request: { params: idp, body: { content: { 'application/json': { schema: MessageBody } } } }, responses: { 200: { description: 'Answer recorded; next question or intakeComplete', content: { 'application/json': { schema: Envelope(Turn) } } }, 429: { description: 'Per-session answer limit or shared public-demo Requestor AI/start budget exhausted', content: { 'application/json': { schema: ErrorEnvelope } } } } });
registry.registerPath({ method: 'post', path: '/assessments/{id}/submit', security: secured, request: { params: idp }, responses: { 200: { description: 'Result computed (rules → scoring → explanation)', content: { 'application/json': { schema: Envelope(Assessment) } } }, 422: { description: 'MISSING_REQUIRED_FACTS', content: { 'application/json': { schema: ErrorEnvelope } } }, 429: { description: 'Shared public-demo Requestor AI/start budget exhausted', content: { 'application/json': { schema: ErrorEnvelope } } } } });
...
registry.registerPath({ method: 'get', path: '/reports/{type}', security: secured, request: { params: ReportParams, query: ReportQuery.innerType() }, responses: { 200: { description: 'Standard report (FR-26); public-demo assessment data and cache entries are isolated to the visitor session or untagged seeded history', content: { 'application/json': { schema: Envelope(ReportResult) } } } } });
registry.registerPath({ method: 'get', path: '/reports/{type}/export', security: secured, request: { params: ReportParams, query: ExportQuery }, responses: { 200: { description: 'CSV or PDF aggregate table rows (FR-28); override-reason free text is never exported', content: { 'text/csv': { schema: z.string() }, 'application/pdf': { schema: z.string().openapi({ format: 'binary' }) } } } } });
registry.registerPath({ method: 'get', path: '/analytics/trends', security: secured, request: { query: TrendsQuery }, responses: { 200: { description: 'Trend rows per period × department | persona | scenario, with public-demo visitor/seeded isolation (FR-27, DASH-03)', content: { 'application/json': { schema: Envelope(ReportResult) } } } } });

registry.registerPath({ method: 'get', path: '/auth/sso/lookup', request: { query: SsoLookupQuery }, responses: { 200: { description: 'SSO provider for the email domain (FR-03); the public-demo tenant is never eligible for lookup/JIT', content: { 'application/json': { schema: Envelope(z.object({ providerId: z.string().nullable(), tenant: z.string().nullable() })) } } } } });
...
registry.registerPath({ method: 'patch', path: '/system/tenant', security: secured, request: { body: { content: { 'application/json': { schema: TenantPatch } } } }, responses: { 200: { description: 'Updated settings; the public-demo tenant stays PAID and may configure only reserved .invalid SSO domains', content: { 'application/json': { schema: Envelope(TenantSettings) } } } } });
...
registry.registerPath({ method: 'post', path: '/system/users', security: secured, request: { body: { content: { 'application/json': { schema: SystemUserCreate } } } }, responses: { 201: { description: 'Pre-provisioned tenant user with exactly one role; sandbox-created users require @demo.invalid and can never become standard login identities', content: { 'application/json': { schema: Envelope(SystemUser) } } } } });
