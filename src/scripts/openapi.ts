...
registry.registerPath({ method: 'get', path: '/assessments', security: secured, request: { query: AssessmentListQuery }, responses: { 200: { description: 'Review dashboard (DASH-01): scoped, filtered, paginated; counts and average confidence within the non-status filters', content: { 'application/json': { schema: Envelope(z.object({ items: z.array(AssessmentListItem), total: z.number(), page: z.number(), limit: z.number(), pages: z.number(), counts: AssessmentCounts, summary: AssessmentSummary })) } } } } });
